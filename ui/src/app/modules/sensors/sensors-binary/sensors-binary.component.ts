...
export class SensorsBinaryComponent implements OnInit {
  ...
  pmSensor = new FormControl('NextPM_SENSPR');
  ...
  private warpPostToken(): Observable<string> {
    ...
    return new Observable(o => {
      this.binaryService.postToken(this.id, {
        ...
        res=>{o.next(res); this.state = State.DONE},
          err=>{o.error(err); this.state = State.ERROR}
      )
    });
  }
  ...
  private runBinary(token: string, retry: number = 180) {
    setTimeout(async() => {
      try {
        window.open("gmstlauncher:" + token, '_blank');
      } catch (err) {
        if (retry == 0) throw err;
